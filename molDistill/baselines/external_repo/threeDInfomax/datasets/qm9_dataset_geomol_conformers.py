...
class QM9DatasetGeomolConformers(Dataset):
    ...
    def __init__(self, return_types: list = None,
        ...
        if not os.path.exists(os.path.join(self.qm9_directory, 'processed_geomol_conformers', self.processed_file)):
            self.process()
        data_dict = torch.load(os.path.join(self.qm9_directory, 'processed_geomol_conformers', self.processed_file))
    ...
    def process(self):
        print('processing data from ({}) and saving it to ({})'.format(self.qm9_directory,
                                                                       os.path.join(self.qm9_directory, 'processed_geomol_conformers')))

        # load qm9 data with spatial coordinates
        geomol_structure_paths = os.listdir(os.path.join(self.qm9_directory, 'geomol_predicted_qm9_structures'))
        geomol_structure_smiles = [os.path.splitext(path)[0] for path in geomol_structure_paths]
        ...
        for mol_idx, n_atoms in tqdm(enumerate(data_qm9['N'])):
            ...
            if smiles not in geomol_structure_smiles:
                continue
           #count += 1
           #if count == 40:
           #    break
            ...
            with open(os.path.join(self.qm9_directory, 'geomol_predicted_qm9_structures', smiles + '.pkl'), 'rb') as f:
                loaded = pickle.load(f)
                conf = loaded[0].GetConformer(0)
        ...
        if not os.path.exists(os.path.join(self.qm9_directory, 'processed_geomol_conformers')):
            os.mkdir(os.path.join(self.qm9_directory, 'processed_geomol_conformers'))
        torch.save(data_dict, os.path.join(self.qm9_directory, 'processed_geomol_conformers', self.processed_file))
